...
public class MainController {
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/uploadExcel_tree", method = RequestMethod.POST)
    public Map<String, Object> uploadExcelTree(@RequestParam("excelFile") MultipartFile file) {
        Map<String, Object> result = new HashMap<>();
        int insertCount = 0;
        ...
        try {
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                Map<String, Object> postMap = new HashMap<>();
                postMap.put("treeUrl", safeString(getCellValue(row.getCell(0))));
                postMap.put("treeTitle", safeString(getCellValue(row.getCell(1))));
                postMap.put("treeUploadDt", safeString(getCellValue(row.getCell(2))));
                postMap.put("ownerNm", safeString(getCellValue(row.getCell(3))));
                postMap.put("farmNm", safeString(getCellValue(row.getCell(4))));
                postMap.put("mbtlNo", extractNumber(safeString(getCellValue(row.getCell(5)))));
                postMap.put("farmAddr", safeString(getCellValue(row.getCell(6))));
                postMap.put("farmArea", safeString(getCellValue(row.getCell(7))));
                postMap.put("creId", "excel");
                postDataList.add(postMap);
            }

            workbook.close();
            insertCount = mainService.insertFarmList(postDataList);
